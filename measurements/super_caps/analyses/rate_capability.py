...
class RateCapabilityAnalysis(BaseAnalysis):
    analysis_id = 'rate_capability'
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            if not ds.visible:
                continue
            df = ds.raw_data
            x_col = _find_col(df, ['current density', 'scan rate', 'rate', 'a/g', 'c-rate'])
            y_col = _find_col(df, ['capacitance', 'cap', 'f/g', 'specific'])
            ...
            n = min(len(x), len(y))
            ax.plot(x[:n], y[:n], 'o-', color=ds.color or None, label=ds.display_name, markersize=5)
            has_data = True

        if not has_data:
            ax.text(0.5, 0.5, 'No rate capability data', transform=ax.transAxes,
