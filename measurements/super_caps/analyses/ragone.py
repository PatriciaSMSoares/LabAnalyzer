...
class RagoneAnalysis(BaseAnalysis):
    analysis_id = 'ragone'
    display_name = 'Ragone Plot'
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            if not ds.visible:
                continue
            df = ds.raw_data
            x_col = _find_col(df, ['power density', 'power_density', 'p (w/kg)', 'power'])
            y_col = _find_col(df, ['energy density', 'energy_density', 'e (wh/kg)', 'energy'])
            ...
            n = min(len(x), len(y))
            ax.loglog(x[:n], y[:n], 'o-', color=ds.color or None, label=ds.display_name, markersize=5)
            has_data = True

        if not has_data:
            ax.text(0.5, 0.5, 'No Ragone data available', transform=ax.transAxes,
