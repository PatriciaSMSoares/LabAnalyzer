...
class GCDAnalysis(BaseAnalysis):
    analysis_id = 'gcd'
    display_name = 'Galvanostatic Charge/Discharge'
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            if not ds.visible:
                continue
            df = ds.raw_data
            x_col = _find_col(df, ['time/s', 'time', 't ('])
            y_col = _find_col(df, ['ewe/v', 'ewe', 'voltage', 'potential', 'v ('])
        ...
        ax.set_title('Galvanostatic Charge/Discharge (GCD)')
