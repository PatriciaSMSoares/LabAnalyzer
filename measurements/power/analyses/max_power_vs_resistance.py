...
class MaxPowerVsResistanceAnalysis(BaseAnalysis):
    analysis_id = 'max_power_vs_resistance'
    display_name = 'Max Power vs Resistance'
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            ...
            n = min(len(r), len(p))
            r, p = r[:n], p[:n]
            # Sort by resistance for a clean curve
            idx = np.argsort(r)
            ax.plot(r[idx], p[idx], 'o-', color=ds.color or None, label=ds.display_name,
        ...
        ax.set_title('Max Power vs Resistance')
