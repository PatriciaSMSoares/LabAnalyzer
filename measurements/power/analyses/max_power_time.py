...
class MaxPowerTimeAnalysis(BaseAnalysis):
    analysis_id = 'max_power_time'
    display_name = 'Max Power vs Time'
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            ...
            x, p = x[:n], p[:n]
            # Rolling maximum
            max_p = np.maximum.accumulate(p)
            ax.plot(x, max_p, '-', color=ds.color or None, label=ds.display_name, linewidth=1.2)
            has_data = True
        ...
        ax.set_title('Cumulative Max Power vs Time')
