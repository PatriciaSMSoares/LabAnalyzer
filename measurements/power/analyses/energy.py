from labanalyzer.core.base_analysis import BaseAnalysis
import numpy as np


def _find_col(df, candidates):
    for c in candidates:
        for col in df.columns:
            if c.lower() in col.lower():
                return col
    return None


class EnergyAnalysis(BaseAnalysis):
    analysis_id = 'energy'
    display_name = 'Energy (Cumulative)'

    def render(self, datasets, config, figure, ax=None):
        if ax is None:
            figure.clear()
            ax = figure.add_subplot(111)
        has_data = False
        cfg = config if isinstance(config, dict) else {}

        for ds in datasets:
            if not ds.visible:
                continue
            df = ds.raw_data
            x_col = _find_col(df, ['time', 't ('])
            p_col = _find_col(df, ['power', 'p ('])
            num_cols = df.select_dtypes(include=[np.number]).columns
            if x_col is None and len(num_cols) >= 1:
                x_col = num_cols[0]
            if p_col is None and len(num_cols) >= 2:
                p_col = num_cols[1]
            if x_col is None or p_col is None:
                continue
            x = df[x_col].dropna().values.astype(float)
            p = df[p_col].dropna().values.astype(float)
            n = min(len(x), len(p))
            x, p = x[:n], p[:n]
            # Cumulative energy via trapezoidal integration
            energy = np.cumsum(np.gradient(x) * p)  # Wh if p in W, t in s
            energy_wh = energy / 3600.0
            ax.plot(x, energy_wh, '-', color=ds.color or None, label=ds.display_name, linewidth=1.2)
            has_data = True

        if not has_data:
            ax.text(0.5, 0.5, 'No data available', transform=ax.transAxes,
                    ha='center', va='center', fontsize=12, color='gray')
        ax.set_xlabel('Time (s)')
        ax.set_ylabel('Energy (Wh)')
        ax.set_title('Cumulative Energy')
        if cfg.get('show_legend', True) and has_data:
            ax.legend(fontsize=8)
        if cfg.get('show_grid', True):
            ax.grid(True, alpha=0.3)
        figure.tight_layout()
