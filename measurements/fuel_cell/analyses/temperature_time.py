...
class TemperatureTimeAnalysis(BaseAnalysis):
    analysis_id = 'temperature_time'
    display_name = 'Temperature vs Time'
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            ...
            anode_col = _find_col(df, ['Temp_Anode (C)', 'temp_anode', 'temp anode', 'anode temp'])
            cathode_col = _find_col(df, ['Temp_Cathode (C)', 'temp_cathode', 'temp cathode', 'cathode temp'])
