...
class PolarizationAnalysis(BaseAnalysis):
    analysis_id = 'polarization'
    display_name = 'Polarization Curve'
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            if not ds.visible:
                continue
            df = ds.raw_data
            x_col = _find_col(df, ['current density', 'current_density', 'j (', 'j('])
            y_col = _find_col(df, ['voltage', 'volt', 'v (', 'cell voltage'])
            ...
            n = min(len(x), len(y))
            ax.plot(x[:n], y[:n], 'o-', color=ds.color or None, label=ds.display_name, markersize=4)
            has_data = True
