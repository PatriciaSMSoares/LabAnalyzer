...
class HumidityTimeAnalysis(BaseAnalysis):
    analysis_id = 'humidity_time'
    display_name = 'Humidity vs Time'
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            ...
            rh_anode_col = _find_col(df, ['RH_Anode (%)', 'rh_anode', 'rh anode', 'anode rh'])
            rh_cathode_col = _find_col(df, ['RH_Cathode (%)', 'rh_cathode', 'rh cathode', 'cathode rh'])
            ...
            if rh_anode_col is not None:
                ya = df[rh_anode_col].dropna().values.astype(float)
                n = min(len(x), len(ya))
                ax.plot(x[:n], ya[:n], '-', color=color,
                        label=f'{ds.display_name} Anode', linewidth=1.2)
                has_data = True
            if rh_cathode_col is not None:
                yc = df[rh_cathode_col].dropna().values.astype(float)
                n = min(len(x), len(yc))
        ...
        if not has_data:
            ax.text(0.5, 0.5, 'No humidity data available', transform=ax.transAxes,
