...
class HFRTimeAnalysis(BaseAnalysis):
    analysis_id = 'hfr_time'
    display_name = 'HFR vs Time'
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            ...
            y_col = _find_col(df, ['HFR (mOhm)', 'hfr'])
        ...
        if not has_data:
            ax.text(0.5, 0.5, 'No HFR data available', transform=ax.transAxes,
                    ha='center', va='center', fontsize=12, color='gray')
        ax.set_xlabel('Time (s)')
        ax.set_ylabel('HFR (mΩ)')
        ax.set_title('HFR vs Time')
