...
class DTGAnalysis(BaseAnalysis):
    analysis_id = 'dtg'
    display_name = 'DTG (dM/dT)'
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            ...
            y_col = _find_col(df, ['mass', 'weight', 'mass%'])
            ...
            if smoothing and smoothing.method != 'None':
                dtg = apply_smoothing(dtg, smoothing)
            ax.plot(x, dtg, '-', color=ds.color or None, label=ds.display_name, linewidth=1.5)
            has_data = True
        ...
        ax.set_ylabel('dM/dT (%/°C)')
        ax.set_title('Derivative Thermogravimetry (DTG)')
