...
class TGACurveAnalysis(BaseAnalysis):
    analysis_id = 'tga_curve'
    display_name = 'TGA Curve'
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            if not ds.visible:
                continue
            df = ds.raw_data
            x_col = _find_col(df, ['temperature', 'temp', 't (°c)', '°c'])
            y_col = _find_col(df, ['mass %', 'mass(%)', 'weight %', 'mass', 'weight'])
