...
class VoltageTimeAnalysis(BaseAnalysis):
    analysis_id = 'voltage_time'
    display_name = 'Voltage vs Time'
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            ...
            x_col = _find_col(df, ['time', 't', 't (', 'time (s)'])
            y_col = _find_col(df, ['voltage', 'volt', 'v', 'v (', 'potential'])
