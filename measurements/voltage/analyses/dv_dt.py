...
class DvDtAnalysis(BaseAnalysis):
    analysis_id = 'dv_dt'
    display_name = 'dV/dt'
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            ...
            y_col = _find_col(df, ['voltage', 'volt', 'v ('])
            ...
            dvdt = np.gradient(y, x)
            if smoothing and smoothing.method != 'None':
                dvdt = apply_smoothing(dvdt, smoothing)

            ax.plot(x, dvdt, '-', color=ds.color or None, label=ds.display_name, linewidth=1.2)
            has_data = True
        ...
        ax.set_title('Rate of Voltage Change (dV/dt)')
