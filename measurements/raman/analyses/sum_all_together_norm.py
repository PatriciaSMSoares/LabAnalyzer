...
def _delete_spikes(x,y, window_size=50):

    local_median = sliding_window_view(y,window_shape=window_size).median(axis=-1)
    local_std = sliding_window_view(y,window_shape=window_size).std()

    is_spike = abs(y - local_median) > (5 * local_std)
    ...
    def render(self, datasets, config, figure, ax=None):
        ...
        for ds in datasets:
            ...
            if group_name is None: # acontece apenas no primeiro dataset
                ...
                ax.plot(x_sum, y_sum/np.max(y_sum), '-', color=ds.color or None, label=f"{group_name}", linewidth=1.2)
